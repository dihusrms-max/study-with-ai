...
def main():
    source = ROOT / "candidate_v2"
    destination = ROOT / "dist"
    destination.mkdir(exist_ok=True)
    zip_path = destination / "submit_integrated_v2_candidate_20260922.zip"
    if zip_path.exists():
    ...
    manifest = {"status": "CANDIDATE_PILOT_ONLY_NOT_SUBMISSION_VALIDATED", "files": files,
                "source_zips": {owner: entry["sha256"] for owner, entry in inventory.items()},
                "validation": json.loads((ROOT / "validation/candidate_v2_latest.json").read_text())}
    manifest["api_pilot"] = json.loads((ROOT / "validation/gemma_pilot_latest.json").read_text())
    with zipfile.ZipFile(zip_path, "x", compression=zipfile.ZIP_DEFLATED) as archive:
        for name in FILES:
            archive.write(source / name, name)
    verify_dir = ROOT / "validation" / ("packaged_v2_" + datetime.now().strftime("%Y%m%d_%H%M%S_%f"))
    ...
    (destination / "manifest_v2.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
