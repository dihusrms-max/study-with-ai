...
def run(records, builder, runner, output_dir, identity, *, chunk=128, max_tokens=1536,
    ...
    for begin in range(0, len(pending_records), chunk):
        ...
        for mode, budget in (("primary", max_tokens), ("compact", COMPACT_TOKENS), ("plain", COMPACT_TOKENS)):
            ...
            pending = retry
        # Preserve successful records even when their batch has failed records.
        for rec, decision in zip(batch, decisions):
            if decision is None:
                failures.append(rec["id"])
                continue
            row = core.make_row(rec, decision)
            edits = []
            if rule_corrections:
    ...
    report = {"version": core.VERSION, "mode": "MOCK_ONLY" if runner.is_mock else "FIXED_LLM",
