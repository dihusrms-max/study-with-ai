...
def main():
    ...
    out = ROOT / "validation" / ("candidate_v2_" + datetime.now().strftime("%Y%m%d_%H%M%S_%f"))
    out.mkdir(parents=True)
    env = {k: v for k, v in os.environ.items() if not k.startswith("PPS_")}
    env.update(PYTHONIOENCODING="utf-8", PYTHONDONTWRITEBYTECODE="1")
    tests = subprocess.run([sys.executable, "-m", "unittest", "discover", "-s", str(ROOT / "tests_v2"), "-v"],
    ...
    for name, options in (("C_baseline", ["--pipeline", "baseline"]),
        ...
        command = [sys.executable, str(ROOT / "candidate_v2/script.py"), "--mock", "--data-dir", str(assets),
    ...
    (ROOT / "validation/candidate_v2_latest.json").write_text(json.dumps({"summary": str(out / "summary.json")}, indent=2), encoding="utf-8")
