...
강사님 방법론에 따라 자가라벨은 학습 데이터가 아니라 FP·FN 후보를 찾기 위한 측정 자료로만 사용한다.

- [ ] 새 무작위 표본이며 이전 dev·골든셋·감사 표본과 겹치지 않는가
- [ ] 항목군 분할, 규칙 판정 맹검, 근거문구 원문 검증을 수행했는가
- [ ] 라벨러 합의 셀과 불일치 셀을 구분했는가
- [ ] 불일치 셀을 사람 또는 심판이 확정했는가
- [ ] 부재탐지 항목의 LLM 라벨을 무조건 채택하지 않았는가
- [ ] 확정 셀을 원인 유형으로 묶고 이전 확정 셀 회귀를 확인했는가
- [ ] 라벨 원문과 공고 본문을 GitHub에 올리지 않았는가

`.work`는 개인 실행·검증 공간으로 유지하며, 팀에는 표본 조건·라벨 출처·검증 결과·해시만 공유한다.
