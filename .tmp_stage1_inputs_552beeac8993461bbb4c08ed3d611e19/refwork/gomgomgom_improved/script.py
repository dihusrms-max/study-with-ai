...
class PromptBuilder:
    def __init__(self, data_dir, doc_chars=14000, law_chars=3000):
        ...
        self.system = build_system(self.table)
...
def run(recs, builder, runner, output, chunk_size=64, max_tokens=MAX_TOKENS):
    ...
    for begin in range(0, len(recs), chunk_size):
        batch = recs[begin:begin + chunk_size]
        ...
        log(f"Processed {len(rows)}/{len(recs)} records; elapsed {time.monotonic() - start:.1f}s")
    write_result(rows, recs, output)
    return {"version": VERSION, "mode": "MOCK_ONLY" if runner.is_mock else "FIXED_LLM",
            "records": len(recs), "format_validation": "PASS", "retry_records": recovered,
            "normal_decision_records": len(rows), "response_diagnostics": dict(sorted(counts.items())),
            ...
            "positive_items_without_quote": sum(row[f"v{i}"] == 1 and not row[f"e{i}"]
                for row in rows for i in range(1, 25) if i not in ABSENCE),
