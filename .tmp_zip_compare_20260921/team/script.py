...
from response_protocol import (COMPACT_OUTPUT, COMPACT_TOKENS, Generation,
                               ResponseError, completed_quotes, flag_schema,
                               infer_batch, read_decision)

VERSION = "gomgomgom-recovery-and-eligibility-20260915"
SEED = 20260826
...
class VLLMRunner:
    ...
    def __init__(self, model_dir, max_tokens=MAX_TOKENS, gpu_mem=0.92):
        ...
        self.llm = LLM(model=str(model_dir), tokenizer=str(model_dir), max_model_len=MAX_MODEL_LEN,
                       gpu_memory_utilization=gpu_mem, seed=SEED, tensor_parallel_size=1,
                       dtype="auto", quantization=QUANTIZATION)
...
def write_checkpoint(rows, path):
    """Write completed rows without requiring the full input set.

    This is intentionally only a recovery checkpoint; final output still goes
    through write_result's complete roundtrip validation.
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def run(recs, builder, runner, output, chunk_size=64, max_tokens=MAX_TOKENS,
        skip_failures=False, failures_path=None, recovery_modes=None):
    start = time.monotonic()
    output = Path(output)
    existing = {}
    if output.exists():
        try:
            with output.open(encoding="utf-8", newline="") as f:
                for row in csv.DictReader(f):
                    if row.get("id"):
                        existing[row["id"]] = row
            log(f"Resuming from checkpoint: {len(existing)} record(s)")
        except (OSError, csv.Error, UnicodeError) as exc:
            log(f"Ignoring invalid checkpoint: {exc}")
            existing = {}
    pending_recs = [rec for rec in recs if rec.get("id") not in existing]
    rows, token_counts, failed_ids = list(existing.values()), [], []
    counts = Counter()
    context_full = recovered = 0
    for begin in range(0, len(pending_recs), chunk_size):
        batch = pending_recs[begin:begin + chunk_size]
        decisions = [None] * len(batch)
        evidence = [{} for _ in batch]
        pending = list(range(len(batch)))
        modes = recovery_modes or (("primary", max_tokens), ("compact", COMPACT_TOKENS), ("plain", COMPACT_TOKENS))
        for mode, budget in modes:
            ...
            for idx in pending:
                ...
                if mode == "primary":
                    context_full += int(info["full_document_context"])
            outputs = infer_batch(runner, prepared, mode, counts)
            next_pending = []
            for idx, response in zip(pending, outputs):
                if mode == "primary":
                    evidence[idx] = completed_quotes(response.text)
                try:
                    obj = read_decision(response, mode, parse_response)
                ...
                if mode != "primary":
                    recovered += 1
                    # Only quote cells from this record's earlier response;
                    # make_row performs the exact source substring check.
                    obj["e"].update(evidence[idx])
                decisions[idx] = obj
                counts["validated_" + mode] += 1
            if next_pending:
                log(f"{mode}: {len(next_pending)} response(s) need recovery")
            pending = next_pending
        if pending:
            summary = json.dumps(dict(sorted(counts.items())), separators=(",", ":"))
            if not skip_failures:
                raise RuntimeError(
                    f"{len(pending)} record(s) lack a valid fixed-model decision after distinct recovery modes; "
                    f"no submission written; diagnostics={summary}") from None
            failed_ids.extend(batch[idx]["id"] for idx in pending)
            log(f"Skipped {len(pending)} failed record(s); diagnostics={summary}")
            continue
        # Postprocessing is only reached after every record in this batch has
        # a complete decision from a genuine content-bearing fixed-model call.
        for rec, obj in zip(batch, decisions):
            rows.append(builder.support.apply(rec, make_row(rec, obj)))
        write_checkpoint(rows, output)
        log(f"Processed {len(rows)}/{len(recs)} records; checkpoint saved; elapsed {time.monotonic() - start:.1f}s")
    by_id = {row["id"]: row for row in rows}
    ordered_rows = [by_id[rec["id"]] for rec in recs if rec["id"] in by_id]
    if failed_ids:
        write_checkpoint(ordered_rows, output)
        if failures_path:
            Path(failures_path).write_text("\n".join(failed_ids) + "\n", encoding="utf-8")
    else:
        write_result(ordered_rows, recs, output)
    return {"version": VERSION, "mode": "MOCK_ONLY" if runner.is_mock else "FIXED_LLM",
            "records": len(ordered_rows), "requested_records": len(recs),
            "failed_records": len(failed_ids),
            "format_validation": "PARTIAL" if failed_ids else "PASS", "retry_records": recovered,
            "normal_decision_records": len(ordered_rows), "response_diagnostics": dict(sorted(counts.items())),
            "full_document_contexts": context_full,
            "max_prompt_tokens": max(token_counts, default=0),
            "token_counts_are_estimates": runner.is_mock,
            "positive_items_without_quote": sum(row[f"v{i}"] == 1 and not row[f"e{i}"]
                for row in ordered_rows for i in range(1, 25) if i not in ABSENCE),
            "elapsed_seconds": round(time.monotonic() - start, 3),
            "model_load_seconds": round(runner.load_seconds, 3), "accuracy_evaluated": False}
...
def main(argv=None):
    ...
    parser.add_argument("--doc-chars", type=int, default=14000)
    parser.add_argument("--law-chars", type=int, default=3000)
    parser.add_argument("--mock", action="store_true")
    args = parser.parse_args(argv)
    if args.limit is not None and args.limit <= 0:
        parser.error("--limit must be positive")
    if args.chunk <= 0 or args.doc_chars < 1400 or args.law_chars < 0:
        parser.error("Invalid budget/batch options")

    server_mode = bool(os.environ.get("PPS_MODEL_DIR"))
    if server_mode and (args.mock or args.limit is not None or args.input):
        parser.error("Server mode prohibits mock, limit, and input override")
    if not args.mock:
        for key, override in (("PPS_DATA_DIR", args.data_dir), ("PPS_OUTPUT_DIR", args.output_dir),
                              ("PPS_MODEL_DIR", args.model_dir)):
            if not os.environ.get(key) and not override:
                parser.error(f"Missing {key}; use --mock only for local plumbing tests")
    data = Path(os.environ.get("PPS_DATA_DIR") or args.data_dir or "data")
    out = Path(os.environ.get("PPS_OUTPUT_DIR") or args.output_dir or "output")
    input_path = Path(args.input) if args.input else data / "test.jsonl.gz"
    recs = list(read_records(input_path, args.limit))
    if not recs:
        raise ValueError("Input contains no records")
    log(f"Starting {'MOCK ONLY (not accuracy)' if args.mock else 'fixed-model inference'} for {len(recs)} records")
    builder = PromptBuilder(data, args.doc_chars, args.law_chars)
    runner = MockRunner() if args.mock else VLLMRunner(os.environ.get("PPS_MODEL_DIR") or args.model_dir)
    filename = "mock_submission.csv" if args.mock else "submission.csv"
    report = run(recs, builder, runner, out / filename, args.chunk)
    report_path = out / ("mock_run_report.json" if args.mock else "run_report.json")
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    log("Finished; output format and source quotation checks passed.")
    return 0
